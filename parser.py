...
def parse_file( fname, edges, poly, transform, screen, color, stack ):
    f = open(fname)
    lines = f.readlines()
    ...
    while c < len(lines):
        ...
        if line == 'sphere':
            ...
            add_sphere(temp,
                       ...
                       float(args[3]), step)
            stack.drawPoly(temp, screen, color)
            ...
            add_circle(temp,
                       float(args[0]), float(args[1]), float(args[2]),
                       float(args[3]), step)
            stack.drawLine(temp, screen, color)
            ...
            #matrix_mult(stack.peekMatrix(), t)
            #stack.peekMatrix() = t
            stack.matrixify(t)
